...
std::array<std::array<bool, kBoardW>, kBoardH> TDEngine::compute_placeable_mask() const {
    ...
    // We do one check to make sure there's a path now
    auto base_path = find_path(spawn_x_, spawn_y_, base_x_, base_y_);
    if (base_path.empty()) return mask;
    
    // This optimization might be slightly different than brute force checking every cell if there are multiple enemies.
    // However, the python code checks every cell with can_place_tower which is O(W*H * Pathfind).
    // To be 100% trace equivalent, let's just use can_place_tower logic!
