...
DynamicsNetworkImpl::DynamicsNetworkImpl(const NetworkConfig& cfg) : cfg_(cfg) {
    ...
    conv = register_module("conv", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(
            cfg_.latent_channels + cfg_.action_planes,
            cfg_.hidden_channels,
            3).padding(1)),
        torch::nn::ReLU(),
        torch::nn::Conv2d(torch::nn::Conv2dOptions(
            cfg_.hidden_channels,
            cfg_.latent_channels,
            3).padding(1)),
        torch::nn::ReLU()
    ));
    pool = register_module("pool", torch::nn::AdaptiveAvgPool2d(
        torch::nn::AdaptiveAvgPool2dOptions(std::vector<int64_t>{1, 1})));
    reward_head = register_module("reward_head", torch::nn::Linear(
        cfg_.latent_channels, cfg_.reward_dim));
}

DynamicsOutput DynamicsNetworkImpl::forward(torch::Tensor latent, const std::vector<int>& actions) {
    auto action_planes = action_encoder->forward(actions, latent.device());
    auto next_latent = conv->forward(torch::cat({latent, action_planes}, 1));
