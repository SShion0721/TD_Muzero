...
class MuZeroNetwork:
    def __new__(cls, config):
        if config.network == "fullyconnected":
        ...
        elif config.network == "modern":
            return MuZeroModernNetwork(
                config.observation_shape,
                config.stacked_observations,
                len(config.action_space),
                config.blocks,
            ...
            raise NotImplementedError(
                'The network parameter should be "fullyconnected", "resnet", or "modern".'
            )
...
class DepthwiseSeparableConv(torch.nn.Module):
    """MobileNet-style depthwise separable convolution. ~9x fewer params than standard conv3x3."""
...
class SEBlock(torch.nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.fc = torch.nn.Sequential(
            torch.nn.Linear(channels, mid, bias=False),
            torch.nn.SiLU(),
            torch.nn.Linear(mid, channels, bias=False),
            torch.nn.Sigmoid(),
        )
...
class ModernRepresentationNetwork(torch.nn.Module):
    def __init__(self, observation_shape, stacked_observations, num_blocks, num_channels):
        super().__init__()
        in_channels = observation_shape[0] * (stacked_observations + 1) + stacked_observations
        self.conv_stem = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels, num_channels, 3, padding=1, bias=False),
            torch.nn.GroupNorm(min(8, num_channels), num_channels),
            torch.nn.SiLU(),
        )
        self.blocks = torch.nn.Sequential(
            *[ModernBlock(num_channels) for _ in range(num_blocks)]
        )
...
class ModernDynamicsNetwork(torch.nn.Module):
    def __init__(self, num_blocks, num_channels, reduced_channels_reward,
        ...
        super().__init__()
        # +1 channel for action plane
        self.conv_in = torch.nn.Sequential(
            torch.nn.Conv2d(num_channels + 1, num_channels, 3, padding=1, bias=False),
            torch.nn.GroupNorm(min(8, num_channels), num_channels),
            torch.nn.SiLU(),
        )
        self.blocks = torch.nn.Sequential(
        ...
        # Reward head
        self.reward_conv = torch.nn.Conv2d(num_channels, reduced_channels_reward, 1)
        self.block_output_size_reward = block_output_size_reward
        self.reward_fc = mlp(block_output_size_reward, fc_reward_layers, full_support_size)
    ...
    def forward(self, x):
        ...
        r = r.view(-1, self.block_output_size_reward)
        reward = self.reward_fc(r)
        return state, reward


class ModernPredictionNetwork(torch.nn.Module):
    def __init__(self, action_space_size, num_channels,
                 reduced_channels_value, reduced_channels_policy,
                 fc_value_layers, fc_policy_layers,
                 full_support_size, block_output_size_value, block_output_size_policy):
        super().__init__()
        self.value_head = torch.nn.Sequential(
            torch.nn.Conv2d(num_channels, reduced_channels_value, 1),
            torch.nn.SiLU(),
        )
        self.policy_head = torch.nn.Sequential(
            torch.nn.Conv2d(num_channels, reduced_channels_policy, 1),
            torch.nn.SiLU(),
        )
        self.block_output_size_value = block_output_size_value
        self.block_output_size_policy = block_output_size_policy
        self.fc_value = mlp(block_output_size_value, fc_value_layers, full_support_size)
        self.fc_policy = mlp(block_output_size_policy, fc_policy_layers, action_space_size)
...
class MuZeroModernNetwork(AbstractNetwork):
    """
    Modern lightweight network for MuZero using MobileNetV3-inspired architecture.
    Uses depthwise separable convolutions, SE attention, GroupNorm, and SiLU.
    Significantly faster than ResNet while maintaining representational capacity.
    """
    def __init__(
        self,
        observation_shape,
        ...
        self.representation_network = ModernRepresentationNetwork(
            observation_shape, stacked_observations, num_blocks, num_channels
        )

        self.dynamics_network = ModernDynamicsNetwork(
            num_blocks,
            num_channels,
            reduced_channels_reward,
            fc_reward_layers,
        ...
        self.prediction_network = ModernPredictionNetwork(
            action_space_size,
            num_channels,
            reduced_channels_value,
            reduced_channels_policy,
    ...
    def representation(self, observation):
        ...
        return (encoded_state - min_encoded_state) / scale
    ...
    def dynamics(self, encoded_state, action):
        ...
        scale = max_s - min_s
        scale[scale < 1e-5] += 1e-5
        return (next_encoded_state - min_s) / scale, reward
