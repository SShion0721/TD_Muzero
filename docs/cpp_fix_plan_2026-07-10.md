...
- Reject non-positive node-pool capacities explicitly.
- Compute the exact search node requirement before initial network inference.
- Reject undersized `max_nodes` before partially mutating the search tree or evaluator latent store.
- Prevent individual expansions from partially allocating children.
- Add explicit NaN and infinity tests for value, reward, and policy logits in both initial and recurrent inference.
- Add exact-capacity and one-slot-short node-pool boundary tests.
- Lock single-player backup semantics with a deterministic `reward + discount * value` sign test.

Pending architecture decision:

- The C++ legality head is present in `PredictionNetwork`, but the evaluator/MCTS contract does not expose it and no canonical C++ trainer supervises it.
- Do not use its logits for latent expansion until a legality target, calibration rule, checkpoint version, and fallback behavior are defined.
- Removing it also changes checkpoint compatibility, so that decision belongs to Phase D rather than a correctness hotfix.
...
The C1 and B1 contract pull-request gates passed the complete Torch-disabled CTest suite on both `ubuntu-latest` and `windows-latest`. Push-triggered status visibility remains limited through the connected API, so CI claims in this document are restricted to runs whose jobs were explicitly inspected.
...
- `4d2695f15a5749945b9bcbe04c460cb351e22464` — MCTS capacity preflight, non-finite regressions, and value-sign contract
